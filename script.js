...
// ===== アカウント設定 =====
document.getElementById('authSettingsBtn').addEventListener('click', function () { openSettings(); });
document.getElementById('settingsClose').addEventListener('click', function () { closeSettings(); });
document.getElementById('settingsModal').addEventListener('click', function (e) { if (e.target === this) closeSettings(); });

function openSettings() {
  var nameEl = document.getElementById('settingsCurrentName');
  nameEl.textContent = currentUsername || currentEmail || '未設定';
  checkNameChangeLimit();
...
document.getElementById('settingsNameBtn').addEventListener('click', async function () {
  var newName = document.getElementById('settingsNewName').value.trim();
  if (!newName) { showToast('新しいユーザーネームを入力してください。'); return; }
  if (newName === currentUsername) { showToast('現在と同じユーザーネームです。'); return; }
  var { data: existing } = await supabase.from('profiles').select('id').eq('username', newName).maybeSingle();
  if (existing) { showToast('このユーザーネームは既に使用されています。'); return; }
  var { data: logData } = await supabase.from('action_log').select('created_at').eq('username', currentUsername).eq('action', 'ユーザーネーム変更').order('created_at', { ascending: false }).limit(1);
  if (logData && logData.length) {
    var lastChange = new Date(logData[0].created_at + 'Z');
    var daysPassed = Math.floor((Date.now() - lastChange.getTime()) / (1000 * 60 * 60 * 24));
    if (daysPassed < 30) { showToast('ユーザーネームは月に1回のみ変更できます。（あと' + (30 - daysPassed) + '日）'); return; }
  }
  var { error } = await supabase.from('profiles').update({ username: newName }).eq('id', currentUserId);
  if (error) { showToast('ユーザーネームの変更に失敗しました。'); return; }
  await supabase.from('action_log').insert({ username: newName, action: 'ユーザーネーム変更', cafe_name: currentUsername + ' → ' + newName });
  currentUsername = newName; authUsernameDisplay.textContent = newName;
  document.getElementById('settingsCurrentName').textContent = newName;
  authGreeting.textContent = 'こんにちは、' + newName + 'さん';
  showToast('ユーザーネームを変更しました。');
  checkNameChangeLimit();
});

document.getElementById('settingsDeleteBtn').addEventListener('click', async function () {
  if (!confirm('本当にアカウントを削除しますか？\nこの操作は元に戻せません。\nすべてのデータが失われます。')) return;
  if (!confirm('最終確認：アカウントを削除してよろしいですか？')) return;
  try {
    var { data: sess } = await supabase.auth.getSession();
    var accessToken = sess?.session?.access_token;
    if (!accessToken) { showToast('セッションが切れています。再ログインしてください。'); return; }
    await supabase.from('profiles').delete().eq('id', currentUserId);
    var res = await fetch('https://nwlfxjtunbqjkwpiaury.supabase.co/auth/v1/user', {
      method: 'DELETE', headers: { 'Authorization': 'Bearer ' + accessToken, 'apikey': SUPABASE_ANON_KEY }
    });
    if (!res.ok) { var err = await res.json(); throw new Error(err.msg || err.message || '削除失敗'); }
    await supabase.auth.signOut();
    showToast('アカウントを削除しました。');
  } catch (err) { showToast('アカウント削除に失敗しました: ' + err.message); }
});

async function checkNameChangeLimit() {
  var note = document.getElementById('settingsNameNote');
  var { data: logData } = await supabase.from('action_log').select('created_at').eq('username', currentUsername).eq('action', 'ユーザーネーム変更').order('created_at', { ascending: false }).limit(1);
  if (logData && logData.length) {
    var lastChange = new Date(logData[0].created_at + 'Z');
    var daysPassed = Math.floor((Date.now() - lastChange.getTime()) / (1000 * 60 * 60 * 24));
    if (daysPassed < 30) { note.textContent = '変更可能まであと' + (30 - daysPassed) + '日'; }
    else { note.textContent = '変更可能です'; }
  } else { note.textContent = '変更可能です'; }
}
